...
Every functional API call opens its own connection via :func:`_connect`
(context-managed, closed on return). This is safe for concurrent asyncio MCP
tool calls with no locks, because no connection or cursor is shared across
threads. Do *not* add shared-connection caching without an internal
``threading.Lock``.
"""
...
def _connect(db_path: str | os.PathLike) -> Iterator[sqlite3.Connection]:
    ...
    - ``PRAGMA foreign_keys=ON`` so the ``artifacts.run_id ... REFERENCES
      runs(run_id) ON DELETE CASCADE`` constraint is enforced on hard delete.
